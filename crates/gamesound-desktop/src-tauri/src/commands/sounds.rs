...
pub fn add_sound(
    ...
    let library = state.library.lock().unwrap();
    let active_profile = library.active_profile().map_err(|e| e.to_string())?;
    ...
    let id = library.add_sound(sound).map_err(|e| e.to_string())?;
    ...
    let inserted = sounds
        .iter()
        .find(|s| s.id == id)
        .ok_or("Failed to read back inserted sound")?;
...
pub fn update_sound(
    ...
    library.update_sound(&updated).map_err(|e| e.to_string())?;
...
pub fn delete_sound(state: State<AppState>, id: i64) -> Result<(), String> {
    let library = state.library.lock().unwrap();
    library.remove_sound(id).map_err(|e| e.to_string())
}
...
pub fn list_categories(state: State<AppState>) -> Result<Vec<CategoryInfo>, String> {
    ...
    let categories: Vec<Category> = if let Some(pid) = profile_id {
        library
            .categories_in_profile(pid)
            .map_err(|e| e.to_string())?
    } else {
        library.categories().map_err(|e| e.to_string())?
    };
...
pub fn delete_category(state: State<AppState>, id: i64) -> Result<(), String> {
    let library = state.library.lock().unwrap();
    library.remove_category(id).map_err(|e| e.to_string())
}
